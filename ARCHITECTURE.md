...
| ---------- | -------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- |
| Files      | Read, Write, Edit, Glob, Grep, NotebookEdit              | MVP: **Read**; v1: **Glob**, **Grep**; v2: **Write**, **Edit**; Notebook opcional / tardío                                                         |
...
El subgrafo `Tools_MVP` es el **mínimo** para arrancar; la **prioridad de ampliación** debe seguir §2.1: antes de permitir `bash`+`grep` como atajo, añadir **Glob** y **Grep** (y luego **Write** / **Edit**) para archivos del proyecto.
...
| `orchestrator` → `hooks`                                                                | Sí (v2+, si **D18**)                                                                                                                   | Emitir eventos; recibir resultados antes/después de tools                       |
...
| Fase    | Añadir al repo                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MVP** | `cmd/assistant`, `config`, `llm`, `orchestrator`, `session` mínimo, `tools` (**read** + **web** + **bash** acotado), `permissions` básico, `channel/repl`; **system prompt** con regla §2.1 (D12); **`llm`** con reintentos mínimos en fallos de red / 502–503 (**D22**, [RETRY_LOGIC.md](RETRY_LOGIC.md))                                                                                                                                                                                                                                                                                                 |
| **v1**  | `glob` + `grep` dedicados, `prompt` con capas, compactación seria en `session`, memoria persistente **manual** (tipos + `MEMORY.md`; ver [`MEMORY_SYSTEM.md`](MEMORY_SYSTEM.md)), **pre-clasificador**: patrones peligrosos ampliados + fast paths locales + hook `ToClassifierInput` por tool ([`YOLO_CLASSIFIER.md`](YOLO_CLASSIFIER.md) §6–§8); **`internal/ide`** si **D21** (MCP localhost, lockfiles, notificación mínima tras Write/Edit) — [IDE_BRIDGE.md](IDE_BRIDGE.md), más tests + `testdata`                                                                                                  |
| **v2**  | `write` + `edit` dedicados; menos bash para mutación de archivos; **`internal/mcp`** si **D6** — cliente **stdio** mínimo, tools `mcp__*`, permisos, límites de salida ([MCP.md](MCP.md) §8); **`internal/classifier`** si **D17** (dos etapas, fail-closed, denegaciones); **`internal/hooks`** mínimo si **D18** (`PreToolUse` / `PostToolUse`, `command`, merge con permisos; hooks de proyecto desactivados por defecto); **opcional** [Coordinator Mode](COORDINATOR_MODE.md) **mínimo** (`internal/coordinator`, env + tools capadas en el centro; workers con sesión aislada; sin tmux obligatorio) |
| **v3**  | **`internal/mcp`** ampliado según **D6** (SSE/HTTP/WS, OAuth, resources, pseudo-auth de referencia); `skills`, **memoria automática** ([MEMORY_SYSTEM.md](MEMORY_SYSTEM.md) §6), **agentes custom** Markdown ([CUSTOM_AGENTS.md](CUSTOM_AGENTS.md), **D19**), **`hooks`** ampliados (más eventos, `prompt`/`agent`/`http`, frontmatter skills + agentes — [HOOKS.md](HOOKS.md)); **`internal/plugin`** local/manifiesto + `--plugin-dir` si **D20** ([PLUGINS.md](PLUGINS.md) §8); **`internal/swarm`** (Team/Swarm, mailboxes) si **D16**; posible `pkg/` si publicáis librería                           |
...
| D13 | Perfiles de agente                              | MVP: un solo perfil; v1: **Explore** + **Plan** con política de contexto (omitir reglas pesadas cuando proceda)                                                                                                                                         | **MVP: un perfil** (tipo General-Purpose); v1+ Explore/Plan ([AGENT_PROFILES.md](AGENT_PROFILES.md)) |
| D14 | Memoria persistente                             | Ninguna en MVP / **manual** (tools o CLI) en v1–v2 / **auto-extract** v3+; ruta: `~/.config/assistant/...` vs `.assistant/memory`                                                                                                                       | TBD                               |
...
| D22 | Reintentos LLM                                  | Max intentos por llamada, base/max backoff, tratamiento 429 (`Retry-After`), 529 (¿foreground vs background?), jitter; política distinta Ollama vs cloud ([RETRY_LOGIC.md](RETRY_LOGIC.md))                                                             | **Default MVP:** **3** intentos/llamada, backoff **1s→8s** + jitter, 429 con **Retry-After** — afinar en [RETRY_LOGIC.md](RETRY_LOGIC.md) |
