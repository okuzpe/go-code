...
**Roadmap nuestro:** **goclaw** ya incluye Coordinator **mínimo** (perfil `coordinator`, `spawn_agent` / `stop_task`, sin tmux) — ver `goclaw/internal/coordinator` y [goclaw/docs/D16_COORDINATOR_SKETCH.md](goclaw/docs/D16_COORDINATOR_SKETCH.md). **Swarm:** buzón **mínimo** en disco en `goclaw/internal/swarm` + [goclaw/docs/SWARM.md](goclaw/docs/SWARM.md) (sin tool Agent ni tmux de referencia).
...
| `coordinator`  | **Coordinator Mode:** allowlist estricta de orquestación; **sin** Read/Write/Bash en el rol central; workers con toolbox completo — [COORDINATOR_MODE.md](COORDINATOR_MODE.md) §2. **En goclaw:** `internal/coordinator` + perfil `coordinator` (implementado). | Subagentes / routing en OpenClaw                                                                                                                        |
...
| `agentprofile` (opcional, v1+) | Built-ins ([AGENT_PROFILES.md](AGENT_PROFILES.md)) + descubrimiento de `agents/*.md`, merge por prioridad — [CUSTOM_AGENTS.md](CUSTOM_AGENTS.md)                                                               | **goclaw:** 7 integrados + custom Markdown (`~/.goclaw/agents`, `.goclaw/agents`). Claude Code: varios integrados + custom; OpenClaw: rutas + agentes                                                                                  |
...
| D13 | Perfiles de agente                              | MVP doc histórico: un perfil; v1+ Explore/Plan                                                                                                                                         | **goclaw:** siete built-in (+ `coordinator`) en [goclaw/internal/agents/profile.go](goclaw/internal/agents/profile.go); `-profile` en CLI ([AGENT_PROFILES.md](AGENT_PROFILES.md)) |
...
| D19 | Agentes personalizados                          | Rutas `agents/*.md`; orden de prioridad; flag “sin customs”; CLI list/show; memoria por agente vs [MEMORY_SYSTEM.md](MEMORY_SYSTEM.md); restricciones plugin/worktree                                                                                   | **goclaw:** `~/.goclaw/agents` y `.goclaw/agents` Markdown+YAML; prioridad proyecto > usuario > built-in |
