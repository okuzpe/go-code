...
func ReplHelpMarkdown(env SlashEnv, sess **session.Session, orch *orchestrator.Orchestrator) string {
	...
	b.WriteString("- **TUI transcript** — PgUp/PgDn · Alt+arrows · mouse wheel opt-in (`tui_mouse_scroll` or `GOCLAW_TUI_MOUSE_SCROLL=1`)\n")
	...
	b.WriteString("- `/agents [name]` — list agents or switch (TUI: **Ctrl+P** when bare `/agents`)\n")
	b.WriteString("- `/profile <name>` — switch profile (same as `/agents <name>`)\n")
