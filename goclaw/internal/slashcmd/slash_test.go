...
func TestHandleSlashInit(t *testing.T) {
	...
	require.NoError(t, err)
	require.Contains(t, string(raw), `"agent_profile": "coordinator"`)
...
func TestHandleSlashApplyPlanPreview(t *testing.T) {
	...
	require.Contains(t, out, "/plan run")
	require.True(t, hints.TUIDocOverlay)
	require.Equal(t, "Plan preview", hints.TUIDocTitle)
