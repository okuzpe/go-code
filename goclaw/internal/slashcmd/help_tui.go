...
func TUIHelpShortcutsMarkdown(st icons.Set) string {
	...
	b.WriteString("- **Ctrl+Shift+←** / **Ctrl+Shift+→** — cycle the active profile (order from **tui_profile_cycle** in settings; default general-purpose → plan → explore → coordinator). The top context bar updates immediately with the active profile and model.\n")
	...
	b.WriteString("- **General-purpose** — Direct coding in this session: explore → implement → verify with full tools (no hub). Tool cards stay compact in the transcript; use **Ctrl+T** or `/tools` for the full history.\n")
