...
func TUIHelpShortcutsMarkdown(st icons.Set) string {
	...
	b.WriteString("Ctrl+B browse (↑↓ j/k scroll); PgUp/PgDn and Alt+arrows always. Mouse wheel is opt-in (**tui_mouse_scroll** or **GOCLAW_TUI_MOUSE_SCROLL=1**) so normal terminal selection stays usable. /copy or /export for the full session.\n\n")
	...
	b.WriteString("- **&task** — spawn_agent (worker profile general-purpose; parent must allow spawn_agent)\n")
	...
	b.WriteString("- **Coordinator** — Default hub profile (--profile coordinator). spawn_agent delegates to workers; the parent session has no direct read/write/shell tools. Synthesizes worker results.\n")
	b.WriteString("- **General-purpose** — Direct coding in this session: explore → implement → verify with full tools (no hub). Tool cards: short result lines when useful; **Ctrl+T** for tool history (/tools when plain text is wired).\n")
