...
Hub-and-spoke: the **`coordinator`** profile uses **`spawn_agent`** and **`stop_task`** only. Workers are separate processes/sessions orchestrated from one hub. See [D16_COORDINATOR_SKETCH.md](D16_COORDINATOR_SKETCH.md) and [COORDINATOR_MODE.md](../../COORDINATOR_MODE.md).

## Swarm (minimal implementation)

**Package:** [`internal/swarm`](../internal/swarm/) — a **disk-backed mailbox hub** for peer-style messaging **without** using `spawn_agent`.
