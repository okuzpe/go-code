...
Documento de profundidad ligado a [ARCHITECTURE.md](ARCHITECTURE.md) (**§2 ter**). Resume una estrategia **realista para 2025–2026** en PC tipo **RTX 4050 + 32 GB RAM** y cómo encaja en **nuestro** diseño (CLI/orquestador en **Go**, no Python).
...
- Los **perfiles de agente** ([ARCHITECTURE.md §2.7](ARCHITECTURE.md), [AGENT_PROFILES.md](AGENT_PROFILES.md)) permiten asignar un modelo **más pequeño** al explorador o al “guide” y reservar el modelo grande para el bucle principal; en Ollama son distintos `model` en la misma API.
...
- **Sí:** desarrollo con IA local, automatización, código + herramientas acotadas, sesiones razonables con compactación ([ARCHITECTURE.md §2.5](ARCHITECTURE.md), [CONTEXT_COMPACTION.md](CONTEXT_COMPACTION.md)).
...
### 2.6 Economía por tipo de agente (referencia §2 de [AGENT_PROFILES.md](AGENT_PROFILES.md))
...
| **Guide** (docs) | Web + lectura, `dontAsk` | **3B–7B** según calidad de respuesta; priorizar **baja latencia** | Riesgo: `dontAsk` sin **D17** sólido es peligroso (véase AGENT_PROFILES §5). |
...
No es obligatorio mapear 1:1 “cada perfil = un GGUF distinto”. Con **14B validado**, muchos equipos usan **14B como único motor** + políticas de contexto (AGENT_PROFILES §3); **7B** añade valor sobre todo como **atajo de latencia** o segundo nombre cuando el swap lo toleráis.
...
**Reglas transversales:** políticas **Permissions** ([ARCHITECTURE.md](ARCHITECTURE.md) §2.3) y encaje en el bucle §3, timeouts agresivos, y **no** bloquear el REPL: job en **background** + notificación o path a fichero cuando termine. **Copyright:** uso comercial de salidas y datasets de entrenamiento es zona gris; el propio artículo enlazado insiste en **riesgo legal** y “mantenerse informado”.
...
[Usuario] → internal/channel (REPL/TUI)
         → internal/orchestrator
              ...
              → internal/permissions → internal/tools
                   ...
                   ├── glob, grep  (v1 — regla §2.1 [ARCHITECTURE.md](ARCHITECTURE.md))
                   ├── write, edit  (v2)
                   ├── image optional → A1111 API
                   └── video optional → proceso externo (fase tardía)
```

El **system prompt** debe reforzar [§2.1](ARCHITECTURE.md) (herramientas dedicadas); los modelos locales suelen beneficiarse **más** de esa regla explícita.
