> **Archived (Spanish, 2026):** Long-form design draft; **§** section anchors are preserved for links from other Markdown files. For a short **English** hub see **[architecture.md](../architecture.md)**. For **shipped GoClaw behavior** see **[goclaw/CLAUDE.md](../../goclaw/CLAUDE.md)**.
...
- **Mapa de cobertura ↔ índice [Claude Code Internals](https://claude-code-explain.helmcode.com/):** [docs-map.md](../docs-map.md) — tabla por tema, alcance MVP y orden de lectura para implementar.
- **Hub conceptual (inglés, corto):** [ARCHITECTURE.md](../architecture.md); este archivo conserva el borrador largo y anclas **§**.
- Lista de enlaces y fuentes externas: [references.md](../reference/references.md).
- **OpenClaw (referencia, sin carpeta local):** notas y enlaces upstream → [openclaw-reference.md](../openclaw/openclaw-reference.md); profundidad → [openclaw-gateway-channels.md](../openclaw/openclaw-gateway-channels.md), [openclaw-agents-tools.md](../openclaw/openclaw-agents-tools.md). **Código relacionado en local:** [claw-code/](../claw-code/) (parity/TUI/Rust — no es OpenClaw).
- **Modelos locales (Ollama, límites de hardware, imagen/vídeo como herramientas):** [local-models.md](../reference/local-models.md).
- **Go vs Rust (asistente CLI):** [go-vs-rust-assistant.md](../reference/go-vs-rust-assistant.md).
- **Perfiles de agente (modelo + tools + permisos + contexto):** [agent-profiles.md](../reference/agent-profiles.md).
- **Memoria persistente (entre sesiones, índice + tipos):** [memory-system.md](../reference/memory-system.md).
- **Ventana de contexto y compactación (micro + auto, presupuestos):** [context-compaction.md](../reference/context-compaction.md).
- **Multi-agente: Coordinator Mode vs Team/Swarm (dos topologías):** [coordinator-mode.md](../reference/coordinator-mode.md).
- **Auto-modo / clasificador de seguridad (YOLO Classifier, consulta lateral LLM):** [yolo-classifier.md](../reference/yolo-classifier.md).
- **Hooks (eventos, extensión shell/HTTP/LLM, workspace trust):** [hooks.md](../reference/hooks.md).
- **Agentes personalizados (Markdown + frontmatter, tool Agent):** [custom-agents.md](../reference/custom-agents.md).
- **Plugins (manifiesto + skills/agents/hooks/MCP empaquetados, marketplace):** [plugins.md](../reference/plugins.md).
- **MCP (cliente hacia servidores externos, transportes, permisos):** [mcp.md](../reference/mcp.md).
- **Integración IDE (MCP localhost) vs Bridge remoto (referencia):** [ide-bridge.md](../reference/ide-bridge.md).
- **Tips prácticos del producto de referencia** (costes, límites de memoria, permisos, perfiles): [practical-tips.md](../reference/practical-tips.md).
- **Reintentos y backoff en llamadas al LLM** (429, 529, 5xx, por-invocation): [retry-logic.md](../reference/retry-logic.md).
- **Skills (SKILL.md, v3):** [skills.md](../reference/skills.md); **seguridad shell profunda:** [bash-security.md](../reference/bash-security.md); **costes API cloud:** [costs.md](../reference/costs.md).
- **Contrato herramientas MVP:** [tool-contract.md](../reference/tool-contract.md) — nombres, riesgos, límites, red, presupuesto de bucle.
- Este archivo: análisis, mapa conceptual y **decisiones pendientes** (marcadas como TBD).
- **Criterio transversal:** al iterar esta documentación con el asistente, **mantener coherencia** con el principio de **herramientas dedicadas** (§2.1): el shell es último recurso, no el sustituto de Read/Grep/Write. Los **perfiles de agente** (§2.7) combinan modelo + herramientas + permisos + qué contexto cargar; detalle en [agent-profiles.md](../reference/agent-profiles.md). **Multi-agente:** no mezclar Coordinator hub-and-spoke con Team/Swarm (§2.11). **Auto-modo:** sin clasificador lateral sólido, evitar combinar con permisos demasiado permisivos (**D5**, **D17**, [yolo-classifier.md](../reference/yolo-classifier.md)). **Hooks de proyecto:** tratar `.assistant/` / settings del repo como **no confiables** hasta workspace trust explícito (**D18**, [hooks.md](../reference/hooks.md) §8). **Agentes `.md` en el repo:** mismo criterio — definen tools/MCP/hooks y son **código de configuración ejecutable** vía shell hooks (**D19**, [custom-agents.md](../reference/custom-agents.md) §1). **Plugins de marketplace:** riesgo **supply chain**; allowlist/denylist y validación de manifiesto (**D20**, [plugins.md](../reference/plugins.md) §1). **IDE local:** la integración **MCP localhost** con el editor es **distinta** del **Bridge** remoto a una UI web; planificarlas por separado (**D21**, [ide-bridge.md](../reference/ide-bridge.md)). **MCP §2.8 vs IDE §2.15:** [mcp.md](../reference/mcp.md) describe el cliente hacia **servidores de integración** (stdio/URL); [ide-bridge.md](../reference/ide-bridge.md) el enlace al **editor** — mismo protocolo en muchos casos, **distinto ecosistema de pares**. **Reintentos API** (**D22**, [retry-logic.md](../reference/retry-logic.md)): backoff **por invocación** a `llm`; no confundir con reintentos de `web_fetch` ni con bucles del orquestador.
...
- **Integración con el editor (local):** el usuario debe poder enlazar el asistente con **VS Code / Cursor / Windsurf** (y, en horizonte, JetBrains) mediante **MCP sobre localhost** — descubrimiento vía lockfiles, sincronización de ediciones y contexto; ver [ide-bridge.md](../reference/ide-bridge.md) y **D21**. No exige paridad con una extensión concreta de terceros: sí un **contrato documentado** y un camino de referencia.
- **Objetivo de despliegue frecuente:** poder usar **modelos locales** en la propia máquina (p. ej. vía **Ollama**) además de, o en lugar de, APIs remotas; el diseño modular (§3–§4) ya lo permite sustituyendo la implementación de `internal/llm`. Detalle y checklist: [local-models.md](../reference/local-models.md).

**Fuera de alcance hasta decidir explícitamente**: paridad 1:1 con Claude Code, **Bridge remoto** análogo a claude.ai (OAuth + backend del proveedor; ver [ide-bridge.md §3](../reference/ide-bridge.md)), telemetría avanzada, sandbox OS en todas las plataformas.
...
| Agents     | Agent, SendMessage, TeamCreate, TeamDelete, Task\*, …    | Post-MVP: **§2.11** distingue **Coordinator** (solo orquestación) vs **Team/Swarm** (peer + mailboxes); [coordinator-mode.md](../reference/coordinator-mode.md) |
...
| MCP        | MCPTool, ListMcpResources, ReadMcpResource, McpAuth      | **v2+** según **D6** — expectativa de mercado: agentes “serios” integran MCP antes que skills/plugins ([mcp.md](../reference/mcp.md) §8)                        |
...
**Nota sobre modelos locales:** cuanto menor sea el modelo, **más** ayuda una lista corta de herramientas bien nombradas y la regla explícita en el prompt; abusar de Bash suele empeorar la tasa de éxito (ver [local-models.md](../reference/local-models.md) D2).
...
- En productos maduros, el código que llama al modelo concentra **reintentos con backoff**, límites de contexto y streaming; conviene concentrarlo en **`internal/llm`** (política por código HTTP, presupuesto **por llamada**, no global — [retry-logic.md](../reference/retry-logic.md)).
...
- Capas independientes: validación de comando, **permisos** (§2.3), rutas, archivos sensibles, opcionalmente **sandbox**, y —en auto-modo— **clasificador / patrones** ([yolo-classifier.md](../reference/yolo-classifier.md)); en muchos entornos Windows la sandbox OS es la capa más difícil; conviene documentar supuestos.
...
- Profundidad y capas tipo producto maduro: [bash-security.md](../reference/bash-security.md) (**D4**).

### 2.5 Contexto largo: compactación

El producto de referencia **gestiona activamente** la ventana (no solo “falla cuando se llena”). Patrón analizado en terceros: [Context & Compaction — claude-code-explain](https://claude-code-explain.helmcode.com/context-compaction). Profundidad y eco Go: [context-compaction.md](../reference/context-compaction.md).
...
**Ventana / overrides (referencia):** ~200K por defecto; modos ~1M en ciertas familias de modelo; override vía config tipo `MAX_CONTEXT_TOKENS`. **Nosotros:** el techo real depende de API u Ollama (véase [local-models.md](../reference/local-models.md)); conviene umbrales **proporcionales** o por tabla de proveedor (**D15**).

**Salida del modelo por fase (referencia):** máximos distintos para turno normal, turno con slots reservados, recuperación tras error y **salida del agente de compactación** (~20K para el resumen) — detalle en [context-compaction.md §5](../reference/context-compaction.md).
...
**Relación con memoria (§2.10):** la compactación recorta el **hilo**; [memory-system.md](../reference/memory-system.md) conserva hechos **estables** fuera del transcript.
...
- Referencia terceros (profundidad): [System Prompt — claude-code-explain](https://claude-code-explain.helmcode.com/system-prompt). Índice local ↔ explainer: [docs-map.md](../docs-map.md) fila 01.

### 2.7 Perfiles de agente (tipo “Agent System”) y multi-agente

Este bloque es **tan importante** como §2.1: en productos maduros cada “agente” no es un modelo suelto, sino un **perfil** con **modelo**, **subconjunto de herramientas**, **modo de permisos** y **qué contexto se inyecta** (reglas del repo, `git status`, etc.). Referencia analizada en terceros: [Agents — claude-code-explain](https://claude-code-explain.helmcode.com/agents). Tabla completa de los **7** perfiles built-in y eco Go: [agent-profiles.md](../reference/agent-profiles.md). Esos tipos son **built-in**; encima conviven **definiciones en Markdown** (un fichero = un agente) — §2.13 y [custom-agents.md](../reference/custom-agents.md).
...
- **Coordinator hub-and-spoke** y **Team/Swarm peer-to-peer** son **dos diseños distintos** (activación, tools del “centro”, mensajería, UI). Resumen canónico: **§2.11** y [coordinator-mode.md](../reference/coordinator-mode.md). No confundir “modo coordinador” (coordinador **sin** lectura/escritura en disco) con un team lead que lleva toolbox completo.
...
**Implementación Go (orientativa)** ver §4.1 (`coordinator` / `swarm`, perfiles) y [agent-profiles.md §4](../reference/agent-profiles.md).

### 2.8 MCP (Model Context Protocol)

Patrón analizado en terceros: [MCP — claude-code-explain](https://claude-code-explain.helmcode.com/mcp). Detalle: [mcp.md](../reference/mcp.md).
...
**Eco Go:** paquete `internal/mcp` ([mcp.md](../reference/mcp.md) §8); mismo pipeline `permissions` que `internal/tools`; [MCP Go SDK](https://github.com/modelcontextprotocol/go-sdk) como base plausible.
...
- En el producto analizado el frontmatter puede declarar **hooks en sesión** (`PostToolUse`, etc.); mismo sistema de eventos que §2.12 — [hooks.md §1](../reference/hooks.md).
- Encaja como capa de producto **después** del MVP de chat + web + shell establecido.
- Contrato resumido: [skills.md](../reference/skills.md).

### 2.10 Memoria persistente (entre sesiones)

**Sí aporta** al producto, pero **después del MVP**: complementa a `session` (§2.5), que solo vive en la conversación actual. Patrón analizado en terceros: [Memory — claude-code-explain](https://claude-code-explain.helmcode.com/memory). Detalle: [memory-system.md](../reference/memory-system.md).
...
- Hechos deducibles del código o de git → no duplicar en memoria (lista explícita en [memory-system.md §5](../reference/memory-system.md)).

**Extracción automática (v2+)**  
Sub-agente o tarea en background tras turnos “silenciosos”, permisos **restringidos** a escribir solo en el árbol de memoria; tope de turnos del extractor; deduplicación — ver [memory-system.md §6](../reference/memory-system.md).
...
**Activación (referencia):** Coordinator → flag de build + env `CLAUDE_CODE_COORDINATOR_MODE=1`; Team/Swarm → tool Agent con **`team_name`**. Fases (research → synthesis → implementation → verification), continue vs spawn, XML `task-notification`, mailboxes y routing: [coordinator-mode.md](../reference/coordinator-mode.md). Terceros: [Coordinator Mode — claude-code-explain](https://claude-code-explain.helmcode.com/coordinator-mode).

**Roadmap nuestro:** **goclaw** ya incluye Coordinator **mínimo** (perfil `coordinator`, `spawn_agent` / `stop_task`, sin tmux) — ver `goclaw/internal/coordinator` y [docs/goclaw/coordinator.md](../goclaw/coordinator.md). **Swarm:** buzón **mínimo** en disco en `goclaw/internal/swarm` + [docs/goclaw/swarm.md](../goclaw/swarm.md) (sin tool Agent ni tmux de referencia).

### 2.12 Hooks (automatización por eventos)

Patrón analizado en terceros: [Hooks — claude-code-explain](https://claude-code-explain.helmcode.com/hooks). Detalle: [hooks.md](../reference/hooks.md).
...
Patrón analizado en terceros: [Custom Agents — claude-code-explain](https://claude-code-explain.helmcode.com/custom-agents). Detalle: [custom-agents.md](../reference/custom-agents.md).
...
**Por qué es interesante implementarlo:** misma abstracción que [agent-profiles.md](../reference/agent-profiles.md) pero **versionable en repo** y editable por equipos; **description** guía la selección del modelo (“use when…”); encaja con [coordinator-mode.md](../reference/coordinator-mode.md) (workers especializados), [hooks.md](../reference/hooks.md) (hooks por agente, lifecycle acotado), memoria **por agente** (scopes `user` / `project` / `local` — distintos de §2.10, ver CUSTOM_AGENTS §5).
...
Patrón analizado en terceros: [Plugins — claude-code-explain](https://claude-code-explain.helmcode.com/plugins). Detalle: [plugins.md](../reference/plugins.md).

**Qué aporta:** un **solo paquete instalable** puede combinar comandos `/`, skills, agentes `.md`, estilos de salida, hooks, servidores MCP/LSP, canales y overrides de settings — con **`userConfig`** (opciones al habilitar) y variables para plantillas. Encaja como **capa de distribución** encima de [custom-agents.md](../reference/custom-agents.md), [hooks.md](../reference/hooks.md), §2.8 MCP y §2.9 skills sin duplicar el modelo mental de cada uno.
...
Patrón analizado en terceros: [Bridge & IDE — claude-code-explain](https://claude-code-explain.helmcode.com/bridge-ide). Detalle: [ide-bridge.md](../reference/ide-bridge.md).
...
Patrón recogido en terceros: [Practical Tips — claude-code-explain](https://claude-code-explain.helmcode.com/tips). Detalle: [practical-tips.md](../reference/practical-tips.md).
...
**Para qué sirve este doc:** checklist rápido al **diseñar** el propio CLI (qué documentar al usuario, qué flags exponer, qué límites fijar). No sustituye [memory-system.md](../reference/memory-system.md), [context-compaction.md](../reference/context-compaction.md), [yolo-classifier.md](../reference/yolo-classifier.md), [agent-profiles.md](../reference/agent-profiles.md).

### 2.17 Reintentos y backoff (API del modelo)

Patrón analizado en terceros: [Retry Logic — claude-code-explain](https://claude-code-explain.helmcode.com/retry-logic). Detalle: [retry-logic.md](../reference/retry-logic.md).
...
**Qué no es:** no sustituye **iron gate** del clasificador ni errores **4xx** de cliente (auth); las llamadas **laterales** (YOLO, compactación) deben tener la misma disciplina pero **no bloquear** con reintentos infinitos ([yolo-classifier.md](../reference/yolo-classifier.md) §9).
...
Referencia índice: [claude-code-explain.helmcode.com](https://claude-code-explain.helmcode.com/). Mapa 1:1: [docs-map.md](../docs-map.md).

| Tema (helmcode)                                                                                                     | En nuestro repo                                              | Alcance MVP                                                                                       |
| ------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------ | ------------------------------------------------------------------------------------------------- |
| **System prompt** (6 capas, caché)                                                                                  | §2.6 + [docs-map.md](../docs-map.md)                            | sí: capas mínimas en `internal/prompt`; sin paridad 6 capas el día uno                            |
...
| **Slash commands** (~100 en referencia) ([slash-commands](https://claude-code-explain.helmcode.com/slash-commands)) | [plugins.md](../reference/plugins.md) (patrón `commands/`); §2.14         | **deferred** — MVP REPL; v3+ plugins/comandos                                                     |
| **Permissions** (6 modos, tabla completa)                                                                           | §2.3 + [yolo-classifier.md](../reference/yolo-classifier.md)              | sí comportamiento; **D5** / **D17** fijan modos                                                   |
| **Tools** (45+ en referencia)                                                                                       | §2.1                                                         | sí subset MVP en §4.4                                                                             |
| **Bash security** (22+ validadores)                                                                                 | §2.4 + [bash-security.md](../reference/bash-security.md)                  | sí política mínima; paridad validadores **no** MVP                                                |
| **Costs** / **fast**                                                                                                | [costs.md](../reference/costs.md), [practical-tips.md](../reference/practical-tips.md) | **N/A** local; sí si D1 cloud                                                                     |

**Para IA:** si no aparece en la tabla como **sí** o **deferred** en MVP, **no** asumir que está en el primer binario sin leer §4.4 y [docs-map.md](../docs-map.md).
...
| --------------------------------------------------------------------- | -------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------- |
| Árbol del monorepo y qué ignorar en v1                                | [openclaw-reference.md](../openclaw/openclaw-reference.md)                 | Mapa + tabla `src/`                                                                                                             |
| Gateway, daemon, canales, routing                                     | `src/gateway/`, `src/channels/`, `src/daemon/`, `src/routing/` | [openclaw-gateway-channels.md](../openclaw/openclaw-gateway-channels.md)                                                                    |
| Herramientas web (SSRF, fetch), agente, auto-respuesta, contexto, MCP | `src/agents/tools/`, `auto-reply/`, `context-engine/`, `mcp/`  | [openclaw-agents-tools.md](../openclaw/openclaw-agents-tools.md)                                                                    |
| Hooks de ciclo de vida (eventos, skills bundled)                      | `src/hooks/`                                                   | [openclaw-agents-tools.md §8](../openclaw/openclaw-agents-tools.md) + [hooks.md](../reference/hooks.md)                                          |
| Extensiones empaquetadas (skills npm / ClawHub, plugins)              | `extensions/`, `skills/`, docs de producto                     | [plugins.md](../reference/plugins.md) (patrón CC); OpenClaw **ClawHub** en práctica                                                          |
| IDE / gateway (MCP, bridge protocol docs)                             | `docs/gateway/`, protocolo bridge en docs                      | [ide-bridge.md](../reference/ide-bridge.md): nosotros priorizamos **MCP localhost** hacia el editor; OpenClaw puede documentar bridge aparte |
...
Resumen de una estrategia **realista** alineada con lo que suele usarse en 2025–2026 en equipos con **GPU media y 32 GB RAM** (p. ej. RTX 4050). El análisis completo, checklist y mapeo “goclaw → paquetes” están en [local-models.md](../reference/local-models.md).
...
**Guía de diseño** — la columna “Eco OpenClaw” enlaza el concepto con el mapa **upstream** en [openclaw-reference.md](../openclaw/openclaw-reference.md); no implica paridad de funciones ni carpeta `openclaw/` en el workspace.
...
| `llm`                          | Cliente HTTP(s): API remota **y/o** backend local (**Ollama**, etc.); **reintentos/backoff** por política (**D22**, [retry-logic.md](../reference/retry-logic.md)); serialización de `tool_use`                                   | Proveedores en `extensions/*`                                                                                                                           |
...
| `permissions`                  | `Allow` / `Ask` / `Deny`, modos, interacción con stdin; orquesta fast paths y (v2+) **clasificador lateral** — [yolo-classifier.md](../reference/yolo-classifier.md)                                                              | `src/security/` + reglas por herramienta                                                                                                                |
...
| `session`                      | Lista de mensajes, límites, **micro-compact**, **resúmenes** (umbrales y políticas: [context-compaction.md](../reference/context-compaction.md))                                                                                  | `src/context-engine/`, `src/sessions/`                                                                                                                  |
...
| `mcp` (v2+, **D6**)            | Cliente MCP: transportes, sesiones, tools `mcp__*`, auth, límites; integración `permissions`                                                                                                                         | OpenClaw: `src/mcp/` (no hay `src/services/mcp/` en el árbol actual); producto tipo Claude Code puede organizar MCP en otra ruta — ver [mcp.md](../reference/mcp.md) |
| `skills` (opcional)            | Descubrir `SKILL.md`, frontmatter, inyección en contexto                                                                                                                                                             | `src/agents/skills/`, carpeta `skills/`                                                                                                                 |
| `coordinator`  | **Coordinator Mode:** allowlist estricta de orquestación; **sin** Read/Write/Bash en el rol central; workers con toolbox completo — [coordinator-mode.md](../reference/coordinator-mode.md) §2. **En goclaw:** `internal/coordinator` + perfil `coordinator` (implementado). | Subagentes / routing en OpenClaw                                                                                                                        |
...
| `agentprofile` (opcional, v1+) | Built-ins ([agent-profiles.md](../reference/agent-profiles.md)) + descubrimiento de `agents/*.md`, merge por prioridad — [custom-agents.md](../reference/custom-agents.md)                                                               | **goclaw:** 7 integrados + custom Markdown (`~/.goclaw/agents`, `.goclaw/agents`). Claude Code: varios integrados + custom; OpenClaw: rutas + agentes                                                                                  |
...
| `hooks` (opcional, v2+)        | Registro por evento; ejecutar `command` / HTTP / (v3+) prompt+agent; fusión con `permissions`; fuentes usuario vs proyecto bajo **D18**                                                                              | `src/hooks/` en OpenClaw; [hooks.md](../reference/hooks.md)                                                                                                          |
| `plugin` (opcional, v3+)       | Descubrir manifiesto, auto-detect `commands                                                                                                                                                                          | agents                                                                                                                                                  | skills  | …`, merge MCP/hooks/agentes; políticas **D20**; sin importar `orchestrator` | Empaquetado tipo CC; ClawHub/extensiones en OpenClaw; [plugins.md](../reference/plugins.md) |
| `ide` (v1+, **D21**)           | Lockfiles → descubrir IDE; cliente MCP **hacia localhost** (SSE/WS); notificaciones de edición/diff; contexto editor → orquestador                                                                                   | Extensión VS Code / plugin JetBrains en referencia CC; [ide-bridge.md](../reference/ide-bridge.md)                                                                   |
...
│   ├── coordinator/              # goclaw: hub-and-spoke mínimo (D16); referencia v3+ Team/Swarm en swarm/
...
│   ├── ide/                      # v1+ opcional pero prioridad producto (MCP hacia editor; ide-bridge.md)
│   └── channel/
...
| Fase    | Añadir al repo                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| ------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **MVP (goclaw)** | **Binario:** [goclaw/cmd/goclaw](../../goclaw/cmd/goclaw/main.go). **Paquetes:** `internal/config`, `llm`, `orchestrator`, `session`, `tools`, `permissions`, `hooks`, `agents`, `memory`. **Tools:** `read_file`, `glob`, `grep`, `bash`, `web_fetch`, `web_search` — [tool-contract.md](../reference/tool-contract.md). **REPL** + slash (`/help`, `/memory`, `/compact`, …). **Sesión** JSONL; **compactación** con umbral por estimación de tokens; **memoria** filesystem + índice; **reintentos LLM** [D22](../reference/retry-logic.md) en `internal/llm/retry.go`. **Fuente de verdad:** [goclaw/CLAUDE.md](../../goclaw/CLAUDE.md). |
| **v1**  | `internal/prompt` con capas explícitas (si se separa del orquestador); compactación con **resumen LLM** opcional; **pre-clasificador** / fast paths ([`YOLO_CLASSIFIER.md`](../reference/yolo-classifier.md) §6–§8); **`internal/ide`** si **D21** — [ide-bridge.md](../reference/ide-bridge.md); más tests + `testdata`. *(Glob/grep/memoria compactación básica y hooks in-process ya en goclaw.)*                                                                                                  |
| **v2**  | `write` + `edit` dedicados; **`internal/mcp`** si **D6** — [mcp.md](../reference/mcp.md) §8; **`internal/classifier`** si **D17**; **hooks** externos (`command`/`http`, más eventos — [hooks.md](../reference/hooks.md)); **opcional** [Coordinator Mode](../reference/coordinator-mode.md) mínimo |
| **v3**  | **`internal/mcp`** ampliado (SSE/HTTP/WS, OAuth, …); `skills` en producto; **memoria automática** ([memory-system.md](../reference/memory-system.md) §6); **agentes custom** Markdown ([custom-agents.md](../reference/custom-agents.md), **D19**); **`internal/plugin`** si **D20** ([plugins.md](../reference/plugins.md) §8); **`internal/swarm`** si **D16**; posible `pkg/` |
| **v4+** | **Marketplace** remoto, cache de versiones, dependencias entre plugins — [plugins.md](../reference/plugins.md) §4–§5 (**D20**)                                                                                                                                                                                                                                                                                                                                                                                                                                                                                          |
...
- **Contexto:** toda llamada bloqueante (`llm.Complete`, HTTP fetch, `exec`) recibe `context.Context` para cancelación con Ctrl+C; los **reintentos** del LLM deben respetar cancelación y no extenderse más allá del presupuesto (**D22**, [retry-logic.md](../reference/retry-logic.md)).
...
| D1  | Proveedor LLM                                   | Anthropic, OpenAI, **Ollama/local**, híbrido                                                                                                                                                                                                            | **Ollama/local primero** en MVP; cloud **segunda** rama de `internal/llm` o híbrido post-MVP ([local-models.md](../reference/local-models.md)) |
| D2  | Tool calling                                    | Nativo del API vs JSON forzado en prompt (**crítico** si D1=local)                                                                                                                                                                                      | **Nativo** si el backend y **D11** lo permiten; **plan B obligatorio:** JSON/tools en texto parseado por orquestador ([tool-contract.md](../reference/tool-contract.md) §3) |
| D3  | Búsqueda web                                    | API de pago (Brave, Serp, etc.) vs fuentes gratuitas acotadas                                                                                                                                                                                           | **MVP:** `web_search` en **stub** si `WEB_SEARCH_PROVIDER=none` (defecto); integración real **antes de beta** ([tool-contract.md](../reference/tool-contract.md) §1) |
| D4  | Shell en Windows                                | `cmd`, PowerShell, Git Bash, WSL únicamente                                                                                                                                                                                                             | **`ASSISTANT_SHELL`:** Git Bash / `wsl` / PowerShell; sin valor: **PowerShell** restrictivo o error documentado; Unix: `sh`/`bash` ([bash-security.md](../reference/bash-security.md)) |
...
| D6  | Cliente MCP                                     | **Expectativa de producto:** v2 al menos **stdio** + permisos + naming `mcp__*` ([mcp.md](../reference/mcp.md)); v3 para SSE/HTTP/WS + OAuth/XAA según necesidad; scopes config y enterprise — detalle en tabla §5                                                   | TBD                               |
...
| D11 | Modelo local por defecto                        | **14B** coder validado en hardware del equipo (corre bien vía Ollama; puede usar RAM por offload); **7B** opción **rápida** o perfiles ligeros; tag exacto p. ej. `qwen2.5-coder:14b` según librería                                                                 | **14B** principal; **7B** opcional rápido ([local-models.md](../reference/local-models.md) §2.2, §2.5)          |
...
| D13 | Perfiles de agente                              | MVP doc histórico: un perfil; v1+ Explore/Plan                                                                                                                                         | **goclaw:** siete built-in (+ `coordinator`) en [goclaw/internal/agents/profile.go](../../goclaw/internal/agents/profile.go); `-profile` en CLI ([agent-profiles.md](../reference/agent-profiles.md)) |
| D14 | Memoria persistente                             | Ninguna en MVP / **manual** en v1–v2 / **auto-extract** v3+                                                                                                                                                                                       | **goclaw:** manual vía `~/.goclaw/memory/` + REPL `/memory` ([memory-system.md](../reference/memory-system.md)); auto-extract post-MVP |
...
| D16 | Multi-agente                                    | ¿Implementar **Coordinator** en v2 y **Team/Swarm** en v3+? ¿Solo Coordinator? ¿Mailboxes en disco bajo `~/.config/assistant/…`? ¿Backend terminal Windows sin tmux?                                                                                    | **goclaw:** Coordinator hub-and-spoke mínimo (`internal/coordinator`); Team/Swarm **TBD** |
| D17 | Clasificador auto-modo (“YOLO”)                 | ¿v2+ con API lateral? Modos `both`/`fast`/`thinking`; modelo clasificador = principal vs más barato (Ollama); **no** cargar políticas auto desde repo sin validación; iron gate en fallos API                                                           | **goclaw:** puntuación de riesgo local + `yolo_threshold` ([`goclaw/internal/permissions/risk.go`](../../goclaw/internal/permissions/risk.go)); sin API lateral LLM **TBD** |
...
| D19 | Agentes personalizados                          | Rutas `agents/*.md`; orden de prioridad; flag “sin customs”; CLI list/show; memoria por agente vs [memory-system.md](../reference/memory-system.md); restricciones plugin/worktree                                                                                   | **goclaw:** `~/.goclaw/agents` y `.goclaw/agents` Markdown+YAML; prioridad proyecto > usuario > built-in |
| D20 | Plugins                                         | ¿Habilitar `internal/plugin`? ¿Solo path/`--plugin-dir` al inicio o marketplace? `allowedPlugins`/`deniedPlugins`; verificación manifiesto; interacción con MCP manual ([plugins.md](../reference/plugins.md))                                                       | TBD                               |
| D21 | Integración IDE                                 | Ruta de lockfiles (`~/.config/assistant/ide/` vs compatibilidad con convención referencia); transportes soportados (SSE/WS); extensión propia vs adaptador documentado; cuándo notificar diff respecto a `permissions` ([ide-bridge.md](../reference/ide-bridge.md)) | TBD                               |
| D22 | Reintentos LLM                                  | Parámetros de referencia y matices 529: [retry-logic.md](../reference/retry-logic.md)                                                                                                                                                                                             | **goclaw:** hasta **10** intentos, backoff **500 ms → 5 min**, **429** / **503** / **504** + `Retry-After`; red transitoria; ver [`goclaw/internal/llm/retry.go`](../../goclaw/internal/llm/retry.go) |
...
3. **[tool-contract.md](../reference/tool-contract.md):** tabla de herramientas, política red §2, presupuesto bucle §4 — es la referencia de implementación de `internal/tools`.
...
2. **Especificar** el contrato mínimo de herramientas — hecho en [tool-contract.md](../reference/tool-contract.md); revisar tras primera semana de código.
3. **Diseñar** el flujo de permisos en pseudocódigo (una página), incluyendo si en auto-modo habrá **fast paths locales** (v1) y/o **clasificador lateral** (v2+, [yolo-classifier.md](../reference/yolo-classifier.md)) y cómo encajan los **hooks** ([hooks.md](../reference/hooks.md), **D18**) respecto a reglas estáticas y al clasificador.
4. **Implementar** solo después de lo anterior.
5. Si habrá **plugins** ([plugins.md](../reference/plugins.md)): definir **D20** (alcance, políticas, supply chain) antes de cargar extensiones de terceros.
6. **Integración IDE:** cerrar **D21** antes de comprometer UX en VS Code/Cursor — contrato MCP localhost, seguridad y orden respecto a permisos ([ide-bridge.md](../reference/ide-bridge.md)).
7. **Cliente MCP como extensibilidad estándar:** cerrar **D6** al planificar v2 — qué transportes y scopes; el IDE usa MCP como **host** distinto del cliente hacia **servidores** externos ([mcp.md](../reference/mcp.md) vs [ide-bridge.md](../reference/ide-bridge.md)).
8. **Reintentos al LLM:** cerrar **D22** antes de beta pública — backoff, códigos, y que el clasificador no entre en bucle ([retry-logic.md](../reference/retry-logic.md), [yolo-classifier.md](../reference/yolo-classifier.md) §9).
...
| Archivo                                                      | Rol                                                               |
| ------------------------------------------------------------ | ----------------------------------------------------------------- |
| [agent-profiles.md](../reference/agent-profiles.md)                       | Perfiles built-in (modelo + tools + permisos + contexto); **D13** |
| [ARCHITECTURE.md](../architecture.md)                           | Hub / decisiones / §2–§6 (**SSOT**)                               |
| [bash-security.md](../reference/bash-security.md)                         | Shell, capas de defensa, **D4**                                   |
| [context-compaction.md](../reference/context-compaction.md)               | Micro/auto compact, presupuestos; **D15**                         |
| [coordinator-mode.md](../reference/coordinator-mode.md)                   | Coordinator vs Team/Swarm; **D16**                                |
| [costs.md](../reference/costs.md)                                         | Costes API cloud vs local; **D1**                                 |
| [custom-agents.md](../reference/custom-agents.md)                         | Agentes `*.md`; **D19**                                           |
| [docs-map.md](../docs-map.md)                                   | Índice explainer ↔ repo, columna MVP                              |
| [go-vs-rust-assistant.md](../reference/go-vs-rust-assistant.md)           | Comparación de stack para CLI asistente                           |
| [hooks.md](../reference/hooks.md)                                         | Eventos, trust, **D18**                                           |
| [ide-bridge.md](../reference/ide-bridge.md)                               | MCP IDE localhost vs Bridge; **D21**                              |
| [local-models.md](../reference/local-models.md)                           | Ollama, hardware; **D10–D11**                                     |
| [mcp.md](../reference/mcp.md)                                             | Cliente MCP hacia servidores externos; **D6**                     |
| [memory-system.md](../reference/memory-system.md)                         | Memoria entre sesiones; **D14**                                   |
| [openclaw-agents-tools.md](../openclaw/openclaw-agents-tools.md) | Notas: agentes y tubería OpenClaw (**GitHub**, sin clon local)    |
| [openclaw-gateway-channels.md](../openclaw/openclaw-gateway-channels.md) | Notas: gateway y canales upstream                                 |
| [openclaw-reference.md](../openclaw/openclaw-reference.md)               | Mapa upstream `openclaw/openclaw` → eco Go                       |
| [plugins.md](../reference/plugins.md)                                     | Paquetes extensión; **D20**                                       |
| [practical-tips.md](../reference/practical-tips.md)                       | Observaciones UX del producto analizado                           |
| [references.md](../reference/references.md)                               | Enlaces externos + puntero a DOCS_MAP                             |
| [retry-logic.md](../reference/retry-logic.md)                             | Backoff LLM; **D22**                                              |
| [skills.md](../reference/skills.md)                                       | Formato SKILL.md, roadmap v3                                      |
| [tool-contract.md](../reference/tool-contract.md)                         | Contrato herramientas MVP, red, presupuesto bucle                    |
| [yolo-classifier.md](../reference/yolo-classifier.md)                     | Clasificador auto-modo; **D17**                                   |

#### Tier 2 — `claw-code/` (13 ficheros)

Subárbol **aparte** (parity, filosofía, cliente Rust/TUI, etc.). No gobierna el layout del asistente Go; la auditoría aquí es **detectar contradicciones** con [ARCHITECTURE.md](../architecture.md) o [docs-map.md](../docs-map.md) si unifiquéis documentación.
...
Revisión cruzada (**Tier 1** y notas OpenClaw en GitHub): [docs-map.md](../docs-map.md), [references.md](../reference/references.md), este archivo, [local-models.md](../reference/local-models.md), [agent-profiles.md](../reference/agent-profiles.md), [memory-system.md](../reference/memory-system.md), [context-compaction.md](../reference/context-compaction.md), [coordinator-mode.md](../reference/coordinator-mode.md), [yolo-classifier.md](../reference/yolo-classifier.md), [hooks.md](../reference/hooks.md), [custom-agents.md](../reference/custom-agents.md), [openclaw-reference.md](../openclaw/openclaw-reference.md), [openclaw-gateway-channels.md](../openclaw/openclaw-gateway-channels.md), [openclaw-agents-tools.md](../openclaw/openclaw-agents-tools.md), [mcp.md](../reference/mcp.md), [plugins.md](../reference/plugins.md), [ide-bridge.md](../reference/ide-bridge.md), [practical-tips.md](../reference/practical-tips.md), [retry-logic.md](../reference/retry-logic.md), [skills.md](../reference/skills.md), [tool-contract.md](../reference/tool-contract.md), [bash-security.md](../reference/bash-security.md), [costs.md](../reference/costs.md).
...
- **Modelos locales:** [local-models.md](../reference/local-models.md) acota Ollama, tamaños de modelo, límites de hardware y separación LLM vs herramientas de media.
- **Perfiles de agente:** [agent-profiles.md](../reference/agent-profiles.md) y §2.7 (modelo + tools + permisos + contexto).
- **Memoria persistente:** [memory-system.md](../reference/memory-system.md) y §2.10 (índice, tipos, límites, anti-patrones).
- **Contexto y compactación:** [context-compaction.md](../reference/context-compaction.md) y §2.5 (micro vs auto, números de referencia, D15).
- **Coordinator vs Team/Swarm:** [coordinator-mode.md](../reference/coordinator-mode.md) y §2.11 (dos topologías, invariante de prompts autocontenidos, **D16**).
- **Clasificador auto-modo:** [yolo-classifier.md](../reference/yolo-classifier.md) y §2.3–§2.4 (consulta lateral, transcript sin texto del asistente, **D17**).
- **Hooks:** [hooks.md](../reference/hooks.md) y §2.9 (skills) + §2.12 (eventos, trust, **D18**).
- **Agentes personalizados:** [custom-agents.md](../reference/custom-agents.md) y §2.7 + §2.13 (**D19**).
- **Plugins:** [plugins.md](../reference/plugins.md) y §2.14 (**D20**).
- **IDE vs Bridge:** [ide-bridge.md](../reference/ide-bridge.md) y §2.15 (**D21**): MCP localhost priorizado; bridge remoto vendor fuera de alcance inicial.
- **MCP cliente (servidores externos):** [mcp.md](../reference/mcp.md) y §2.8 (**D6**): expectativa v2+; no confundir con el rol MCP del **host IDE** (§2.15).
- **Tips prácticos / UX esperable:** [practical-tips.md](../reference/practical-tips.md) y §2.16 (costes `/fast`, límites `MEMORY.md`, `bypassPermissions`, Verification).
- **Reintentos LLM:** [retry-logic.md](../reference/retry-logic.md) y §2.17 + **D22** (429/529/5xx, backoff por invocación, unattended).
- **Mapa explainer ↔ repo:** [docs-map.md](../docs-map.md) y §2.18 (KAIROS, hidden, slash, costs, bash); evita que una IA asuma paridad 1:1 con Claude Code en MVP.

### 8.2 Huecos recomendables (imperdibles antes o junto al MVP de código)

| Tema                            | Por qué importa                                                                                                       | Acción sugerida                                                                                                                                                                                     |
| ------------------------------- | --------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Contrato de herramientas v1** | Sin nombres + JSON Schema + límites de salida, el implementador improvisa y el modelo oscila.                         | **[tool-contract.md](../reference/tool-contract.md)** — tabla MVP, §3 **D2**; ampliar JSON Schema en código cuando exista `internal/tools`.                                         |
| **Política de red explícita**   | OpenClaw insiste en SSRF; nuestro §2.4 habla de shell pero el riesgo de **`web_fetch`** es el primer agujero en prod. | **[tool-contract.md](../reference/tool-contract.md) §2** — redirecciones, límites, bloqueo SSRF básico; refinar con tests.                                                                                                       |
| **Límite de turnos / budget**   | El bucle §3.1 puede colgar o gastar tokens sin tope.                                                                  | **[tool-contract.md](../reference/tool-contract.md) §4** — iteraciones LLM y tools; **D15** para ventana y compactación. |
| **Modelo de amenaza mínimo**    | Quién es el adversario (usuario, modelo, sitio web malicioso, repo local) condiciona permisos.                        | Medio pliego: 1 página “Threat model MVP”; alinear categorías BLOCK con [yolo-classifier.md](../reference/yolo-classifier.md) cuando exista auto-modo.                                                           |
...
- **Checklist** al final de openclaw-agents-tools: enlazar [tool-contract.md](../reference/tool-contract.md).
...
- **references.md** y **[docs-map.md](../docs-map.md):** enlaces externos + tabla de cobertura; evitar duplicar párrafos largos de arquitectura en References; el mapa solo enlaza y clasifica MVP.
...
- **Coherencia con §2.10:** no documentar como “memoria” lo que debe ir en **AGENTS.md** o en el repo; mantener la lista §5 de [memory-system.md](../reference/memory-system.md).
...
- **Coherencia §2.8 vs §2.15:** **MCP** en §2.8 es el **cliente hacia servidores de integración** (GitHub, DB, …). El **IDE** en §2.15 usa MCP como **transporte al editor**; son relaciones distintas (véase [mcp.md](../reference/mcp.md) intro y [ide-bridge.md](../reference/ide-bridge.md) §2).
- **Coherencia §2.16:** los “tips” son **observaciones del producto analizado**; al portar decisiones a Go (p. ej. umbral de compactación), validar contra **D15** y hardware local, no copiar cifras literalmente desde [practical-tips.md](../reference/practical-tips.md).
...
- **Coherencia §2.18 y DOCS_MAP:** cualquier tema listado como **no** o **deferred** en MVP no debe implementarse sin actualizar §4.4 y la tabla en [docs-map.md](../docs-map.md).
...
| 2026-04-07 | Stack fijado a **Go** (D0); sección OpenClaw y módulos Go alineados con [openclaw-reference.md](../openclaw/openclaw-reference.md) y documentos de profundidad                                                                                             |
...
| 2026-04-07 | **Modelos locales:** §1 objetivo de despliegue; **§2 ter** encaje + límites hardware; [local-models.md](../reference/local-models.md); decisiones **D10–D11**; `llm` aclarado para Ollama/local                                                             |
| 2026-04-07 | Enlace a [go-vs-rust-assistant.md](../reference/go-vs-rust-assistant.md) en cabecera y changelog                                                                                                                                                            |
...
| 2026-04-07 | **§2.7 perfiles de agente** (6 tipos referencia, roadmap MVP→v1, coordinador); [agent-profiles.md](../reference/agent-profiles.md); §4 `agentprofile/`, **D13**; §8.1 y §8.4                                                                                |
| 2026-04-07 | **§2.10 memoria persistente**; [memory-system.md](../reference/memory-system.md); §2.6 índice en prompt; §3.2 nota Mem; §4 `memory/`, **D14**, §4.3 dep; §8; enlace cabecera                                                                                |
| 2026-04-07 | **§2.5 ampliado** (micro vs auto-compact, ventana, salidas, `/compact`); [context-compaction.md](../reference/context-compaction.md); **D15**; §4.1 `session`; §8.1–§8.2–§8.4; enlace cabecera y References                                                 |
| 2026-04-07 | **§2.11 multi-agente** (Coordinator vs Team/Swarm); [coordinator-mode.md](../reference/coordinator-mode.md); §2.1 taxonomía Agents; §2.7 orquestación; §4.1 `coordinator`+`swarm`, §4.3 árbol; §4.4 v2–v3; **D16**; §8; References                          |
| 2026-04-07 | **§2.3–§2.4** clasificador auto-modo; [yolo-classifier.md](../reference/yolo-classifier.md); §4.1 `permissions`+`classifier`, §4.2 árbol, §4.4 v1–v2; **D17**; §8.1–§8.2–§8.4; cabecera y References                                                        |
| 2026-04-07 | **§2.12 Hooks**; [hooks.md](../reference/hooks.md); §2.9 skills+hooks; §2 bis tabla OpenClaw; §4.1 `hooks`, §4.2–§4.3–§4.4 v2–v3; **D18**; §8; References                                                                                                   |
| 2026-04-07 | **§2.13 agentes personalizados**; [custom-agents.md](../reference/custom-agents.md); §2.7 built-in vs custom; cabecera trust; §4.1 `agentprofile`, §4.2 config paths, §4.4 v3; **D19**; §8.1–§8.4; References                                               |
| 2026-04-07 | **§2.14 Plugins**; [plugins.md](../reference/plugins.md); §2 bis; §4.1 `plugin`, §4.2–§4.3–§4.4 v3–v4; **D20**; §6 paso 5; §8; References                                                                                                                   |
| 2026-04-07 | **§2.15 IDE + Bridge**; [ide-bridge.md](../reference/ide-bridge.md); §1 alcance IDE local vs bridge remoto; **D21**; §4.1 `ide`, §4.2–§4.3–§4.4 v1; §6 paso 6; §8; References; criterio cabecera                                                            |
| 2026-04-07 | **§2.8 MCP ampliado**; [mcp.md](../reference/mcp.md); §2.1 taxonomía MCP v2+; §4.1 `mcp`, §4.2 árbol, §4.3 deps `mcp`, §4.4 v2–v3; **D6** redefinido; §6 paso 7; §8; References; criterio transversal MCP vs IDE                                            |
| 2026-04-07 | **§2.16 Tips prácticos**; [practical-tips.md](../reference/practical-tips.md); cabecera; §8; References; §8.4 coherencia cifras                                                                                                                             |
| 2026-04-07 | **§2.17 Reintentos LLM**; [retry-logic.md](../reference/retry-logic.md); §2.2; §4.1 `llm`, §4.4 MVP, §4.5 context; **D22**; §6 paso 8; §8; References; [yolo-classifier.md](../reference/yolo-classifier.md) §13; criterio transversal reintentos                        |
| 2026-04-07 | **Paridad helmcode / legibilidad IA:** [docs-map.md](../docs-map.md); cabecera mapa+hub; §2.18; §2.4→BASH_SECURITY; §2.9→SKILLS; bullets SKILLS/BASH/COSTS; §8 lista+§8.1+§8.4; [references.md](../reference/references.md) enlaces 01–21                      |
| 2026-04-07 | §2.6: enlace explainer System Prompt + [docs-map.md](../docs-map.md) fila 01; [practical-tips.md](../reference/practical-tips.md) §4 → [costs.md](../reference/costs.md); [openclaw-agents-tools.md](../openclaw/openclaw-agents-tools.md) → DOCS_MAP                           |
| 2026-04-07 | **§8.0 inventario completo** de Markdown (635): Tier 1 raíz 23, Tier 2 `claw-code/` 13, Tier 3 `openclaw/` 599; política de auditoría; §8.4 mantenimiento inventario; [references.md](../reference/references.md) y [docs-map.md](../docs-map.md) enlazan §8.0 |
| 2026-04-07 | **§8.0:** Tier 1 pasado a **24** ficheros (+ [tool-contract.md](../reference/tool-contract.md)); intro y totales con `find` local (sin fijar cifra global si falta `openclaw/`) |
| 2026-04-07 | Auditoría árbol `openclaw/`: [openclaw-reference.md](../openclaw/openclaw-reference.md) alineado al repo real; §4.1 eco `mcp` sin `src/services/mcp/`                                                                                                      |
| 2026-04-07 | **D11:** modelo local **14B** coder validado como principal; **7B** opcional rápido; [local-models.md](../reference/local-models.md) §2.2, §2.5                                                                                                                  |
| 2026-04-07 | **Cierre pre-código:** [tool-contract.md](../reference/tool-contract.md); **§5.1** checklist; **D1–D5, D7–D8, D10, D13, D22** cerrados para MVP; §6 paso 2; §8.0–§8.2–§8.3 actualizados                                                                              |
| 2026-04-07 | **Sin `openclaw/` local:** §2 bis upstream + GitHub; cabecera + Tier 3 §8.0 opcional; `OPENCLAW_*` + [openclaw-reference.md](../openclaw/openclaw-reference.md) enlaces `blob`/`tree`; contraste [claw-code/](../claw-code/); [references.md](../reference/references.md), [docs-map.md](../docs-map.md) |
