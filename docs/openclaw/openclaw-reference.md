...
**GoClaw:** For **shipped CLI behavior** in this repo, use [`goclaw/CLAUDE.md`](../../goclaw/CLAUDE.md). This file is **upstream OpenClaw** reference only (Node/TS product on GitHub).

**Estado en este workspace:** **no** hay carpeta local `openclaw/`. Este archivo y [openclaw-gateway-channels.md](./openclaw-gateway-channels.md), [openclaw-agents-tools.md](./openclaw-agents-tools.md) son **notas de arquitectura** basadas en el producto upstream; los enlaces de rutas apuntan a **[openclaw/openclaw en GitHub](https://github.com/openclaw/openclaw)** (`main`). Sitio de documentación: [docs.openclaw.ai](https://docs.openclaw.ai). OpenClaw **no** es el código que vamos a portar a Go: es **referencia de modularización y riesgos**.
...
- [openclaw-gateway-channels.md](./openclaw-gateway-channels.md) — gateway, daemon, canales y enrutado.
- [openclaw-agents-tools.md](./openclaw-agents-tools.md) — agentes, herramientas, web, sandbox, auto-respuesta, MCP.
...
Hay **decenas** de carpetas bajo `src/`; aquí el subconjunto que **mapea mejor** al MVP Go de [ARCHITECTURE_LEGACY_ES.md §4.1](../archive/architecture-legacy-es.md). El resto (media, voz, canvas, infra…) es producto OpenClaw amplio; no hace falta portarlo para un CLI tipo “Claude Code lite”.
...
| `context-engine/` | Motor de contexto (podado, ensamblaje) | `internal/session`, compactación — [CONTEXT_COMPACTION.md](../reference/context-compaction.md) |
| `cron/` | Tareas programadas | Fase posterior |
| `daemon/` | Servicio en segundo plano | Opcional |
| `hooks/` | Hooks de sesión y eventos (incl. bundled bajo `hooks/bundled/`) | `internal/hooks` — [HOOKS.md](../reference/hooks.md) |
| `mcp/` | Cliente / integración MCP en core | `internal/mcp` (**no** usar la ruta inexistente `src/services/mcp/`) |
| `plugins/` | Runtime de carga de plugins en proceso | `internal/plugin` (modelo distinto en Go) — [PLUGINS.md](../reference/plugins.md) |
...
- **Búsqueda** (`brave`, `duckduckgo`, `searxng`, `tavily`, `exa`) → decisión D3 en [ARCHITECTURE_LEGACY_ES.md](../archive/architecture-legacy-es.md).
...
| 2026-04-07 | Alineación con árbol real: OpenClaw ≠ Claude Code CLI; checkout filtrado; §2 (extensiones ~10², carpetas raíz extra, `src/plugin-sdk/` vs `packages/plugin-sdk/`); §3 (`hooks/`, `commands/`, `mcp/` sin `services/mcp/`, bloque “otros directorios”). Coherencia: [ARCHITECTURE_LEGACY_ES.md §4.1](../archive/architecture-legacy-es.md) fila `mcp` sin ruta inexistente. |
