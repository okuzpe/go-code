...
- **Default `agent_profile` (erratum):** this line described **`general-purpose`** as the global default at release time; **current `config.Default().AgentProfile`** is **`coordinator`** (hub). Use **`/profile general-purpose`** or **`builder`** for direct tools on the main session. See [Unreleased] above, [usage.md — Agent profiles](usage.md#agent-profiles), and [coordinator.md](coordinator.md).
