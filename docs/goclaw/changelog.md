...
- **Orchestrator / UX:** Mid-turn **budget** system-reminder is **softer** when the user asked for code changes, tools already ran, and no successful workspace write has occurred yet (still wraps up normally for explore-style turns). **Auto-continue** now also fires when the **first** completion in a turn has **zero** tool calls (typical local-model “prose only” failure) under the same cap as post-read-only nudges; **`LastUserNaturalText`** skips both nudge templates. More Spanish write-intent keywords in **`action_intent.go`**. Optional **`auto_continue_action_max_nudges`** (1–5, default 2) in `settings.json`. **TUI:** **footer hint** when the truth-on-disk footer appears (mentions **`/continue`**); **`git diff --stat`** as a system line after successful writes (shared **`internal/gitdiff`** helper with readline). **Readline:** stderr **`goclaw:`** recap; same diff helper; truth-footer hint. **Banner:** **`OllamaNumCtxBannerWarnBelow`** (**8192**) for **`ollama_num_ctx`** warning. **Welcome** tips mention turn shape and follow-up **continue**.
