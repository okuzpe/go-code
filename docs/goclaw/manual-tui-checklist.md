...
1. **Fresh settings + TTY** — With no `~/.goclaw/settings.json`, run `goclaw` (or `go run ./cmd/goclaw`) from a project directory. Complete: security screen → trust workspace (option 1) → appearance → **agent profile** (hub `coordinator` vs direct `general-purpose`; **Esc** returns to appearance) → Ollama host/model (Enter through defaults is fine). Confirm `~/.goclaw/settings.json` exists (including `"agent_profile"` matching your choice), and project `.goclaw/settings.json` has `"trusted_workspace": true`. Completion tip should match the chosen profile and mention **`/profile`** to switch later.
