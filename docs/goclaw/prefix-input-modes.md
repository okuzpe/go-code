...
| `&` | `&` + task description (first line) | Runs **`spawn_agent`** with worker profile **`build`** (internal alias: `general-purpose`), `task` set to the text after `&`, default `timeout_sec`, `interactive: false`. Requires **`spawn_agent`** on the active profile’s tool registry (**`coordinator`** includes it; default **`build`** does not — use **`/profile coordinator`** only when you intentionally want hub mode). If the tool is unavailable, the user sees a clear error. |
