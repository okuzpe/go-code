...
| `&` | `&` + task description (first line) | Runs **`spawn_agent`** with `profile: general-purpose`, `task` set to the text after `&`, default `timeout_sec`, `interactive: false`. Requires **`spawn_agent`** on the active profile’s tool registry (**`coordinator`** includes it; default **`general-purpose`** does not — use **`/profile coordinator`** for hub mode). If the tool is unavailable, the user sees a clear error. |
