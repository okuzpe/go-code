...
| `&` | `&` + task description (first line) | Runs **`spawn_agent`** with `profile: general-purpose`, `task` set to the text after `&`, default `timeout_sec`, `interactive: false`. Requires **`spawn_agent`** on the active profile’s tool registry (use **`/profile coordinator`** or hub mode; the default **`general-purpose`** profile does not include **`spawn_agent`**). If the tool is unavailable, the user sees a clear error. |
